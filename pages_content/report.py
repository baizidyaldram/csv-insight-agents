...
def build_full_context(df: pd.DataFrame) -> str:
    ...
    # ML modeling info
    modeling_done = st.session_state.get("modeling_done")
    if modeling_done:
        parts.append(f"\nMachine Learning Predictive Model: {st.session_state.trained_model_name}")
        parts.append(f"Predicting Target: {st.session_state.model_target_col}")
        parts.append(f"Features: {', '.join(st.session_state.model_features_list)}")
...
def generate_markdown_report(df: pd.DataFrame, llm_report: str, file_name: str) -> str:
    ...
    modeling_done = st.session_state.get("modeling_done")
    if modeling_done:
        lines.append("## 🤖 4. Machine Learning Modeling & Evaluation\n")
    ...
    lines.append(f"## 💡 {5 if modeling_done else 4}. AI-Generated Insights\n")
    lines.append(insights_text)
    lines.append("\n")

    lines.append(f"## 📄 {6 if modeling_done else 5}. Detailed Analysis\n")
    lines.append(llm_report)
    lines.append("\n")

    lines.append("---")
    lines.append(f"*Report generated by CSV Agent Analyzer · {now}*")
...
def render():
    ...
    steps = [
        ...
        ("🤖 ML Modeling", st.session_state.get("modeling_done", False)),
